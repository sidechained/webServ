...
void LocationConfig::extractKey(std::string &keyToExtract, std::size_t &colonPos) {
	colonPos = line.find(':');
	if (colonPos == std::string::npos)
		errorExit(ERR_PARSE, ERR_PARSE_NOKEY);
	keyToExtract = line.substr(0, colonPos);
}

void LocationConfig::extractValue(std::string &value, std::size_t &colonPos) {
...
void LocationConfig::errorExit(std::string err1, std::string err2) {
...
bool LocationConfig::countTabIndents(int numTabs) {
	if (line.empty())
		return false;
...
void LocationConfig::detectAndStripTabIndents(int numTabs) {
	if (line.empty())
